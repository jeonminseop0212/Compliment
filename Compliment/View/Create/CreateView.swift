...
class CreateView: UIView {
  ...
  @objc private func tapStart() {
    guard let goalTextFiled = self.goalTextFiled,
          let goalTitle = goalTextFiled.text,
          let dateTextFiled = self.dateTextFiled,
          let date = dateTextFiled.text,
          let intDate = Int(date),
          let rewardTextFiled = self.rewardTextFiled,
          let reward = rewardTextFiled.text else { return }
  ...
  private func goalSave(goal: Goal) {
    do {
      let realm = try Realm()
      try realm.write {
        realm.add(goal)
